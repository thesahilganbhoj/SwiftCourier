...
useEffect(() => {
console.log(`[StaffDetails] Component mounted or staffId changed. Current staffId: ${staffId}`)
fetchPendingOrders()
fetchAcceptedTasks()
...
try {
  console.log(`[StaffDetails] Fetching staff availability for ID: ${staffId}`)
  const availabilityData = await getStaffAvailability(staffId)
  console.log("[StaffDetails] Fetched availability:", availabilityData)
  setAvailability(availabilityData)
} catch (err) {
  console.error("[StaffDetails] Failed to fetch staff availability:", err)
  setError("Failed to load staff availability.")
  // Set a default availability if fetch fails, to prevent UI issues
  setAvailability({
    staffId: staffId,
    isAvailable: false, // Default to unavailable if fetch fails
...
try {
  ...
  console.log(`[StaffDetails] Updating availability for staff ID: ${staffId} with data:`, updateData)

  const response = await updateStaffAvailability(staffId, updateData)
  console.log("[StaffDetails] Availability update response:", response)

  // Update local state
  setAvailability((prev) => ({
    ...prev,
    isAvailable: newAvailabilityStatus,
    lastUpdated: new Date().toLocaleString(), // Update timestamp locally for display
  }))
  ...
  console.error("[StaffDetails] Error updating availability:", err)
    ...
    <div className="row mb-4">
      <div className="col-12 d-flex justify-content-between align-items-center pe-5">
        ...
        <div className="dropdown">
          ...
          <ul className="dropdown-menu" aria-labelledby="availabilityDropdown" data-bs-auto-close="true">
            <li>
              <button
